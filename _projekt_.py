        ...
        if all(elem >= 0 for elem in produkt_a_z):
            ...
            if j_k < n:
                ...
                if all(elem >= 0 for elem in zacasni_Z[0:m]):
                    ...
                    r = r_plus_s
                    vsi_negativni = seznam_indeksov_negativnih_komponent_x
                    while r != 0:
                        ...
                        for i in seznam_indeksov_nenegativnih_komponent_x:
                            vsota = 0
                            x_i = zacasni_Z[i]
                            vsota = x_i + vsota_neg_komponent_x_i / len(seznam_indeksov_nenegativnih_komponent_x)
                            if vsota < 0:
                                Z[i] = 0
                        ...
                        if zacasni_seznam_i_neg_komponent != [] or r != 0:
                            vsi_negativni = [*vsi_negativni, *zacasni_seznam_i_neg_komponent]
                        seznam_indeksov_negativnih_komponent_x = zacasni_seznam_i_neg_komponent
                        seznam_indeksov_nenegativnih_komponent_x = zacasni_seznam_i_neneg_komponent
                        
                        r_plus_s = len(vsi_negativni)
                    vsota_nicelnih_x = 0
                    for i in vsi_negativni:
                        vsota_nicelnih_x = vsota_nicelnih_x + zacasni_Z[i]
                    for i in range(m):
                        if i not in vsi_negativni:
                            Z[i] = zacasni_x[i] + vsota_nicelnih_x / len(seznam_indeksov_nenegativnih_komponent_x)
                    Z[-1] = zacasni_v
                ...
                if all(elem >= 0 for elem in zacasni_Z[m:(m+n)]):
                    ...
                    for i in range(m,(m+n)):
                        if i not in vsi_negativni:
                            Z[i] = zacasni_Z[i] + vsota_nicelnih_x / len(seznam_indeksov_nenegativnih_komponent_x)
                    Z[-1] = zacasni_v
               
   # with open("iteracija2_matrika1_15000.txt", 'w', encoding='utf-8') as izhodna:
        #izhodna.write("{}\n".format(vrednosti))
      
    return(Z[0:m], Z[m:(m+n)], Z[-1], stevilo_iteracij)  
...
def iteracija_brown(matrika, stevilo_iteracij):
    ...
    return([P,Q, vred1, stevilo_iteracij])
...
def resitev_LP(matrika):
    ...
    BY = [1] * n
    meje = [(0,1)]
    resitev2 = linprog(c = CY, A_ub = AY, b_ub = BY, bounds=meje, method="simplex")
...
def analiza2(matrika, stevilo_iteracij):
    ...
    #with open("iteracija_brown__povp.txt", 'w', encoding='utf-8') as izhodna:
           # izhodna.write("{}\n".format(seznam_povp_vrednosti))
    #with open("iteracija_brown_sp.txt", 'w', encoding='utf-8') as izhodna:
           # izhodna.write("{}\n".format(seznam_sp_vrednosti))
   # with open("iteracija_brown_zg.txt", 'w', encoding='utf-8') as izhodna:
           # izhodna.write("{}\n".format(seznam_zg_vrednosti))
    with open("iteracija_brown_napaka.txt", 'w', encoding='utf-8') as izhodna:
        izhodna.write("{}\n".format(napaka))
    with open("iteracija_brown_cas.txt", 'w', encoding='utf-8') as izhodna:
        izhodna.write("{}\n".format(cas))
    return(korak)
